...
<form action="${pageContext.request.contextPath}/hr/postAction_update.action" method="post">
    <%--隐藏提交的表单属性--%>
    <input type="hidden" name="postId" value="${empty post?'-1':post.postId}"/>
            ...
            <td>
                <select name="department.depID"
                    ...
                    <s:iterator value="%{#departments}" var="list">
                        <option value="${list.depID}"  ${list.depID==post.department.depID?'selected':''}>${list.depName}</option>
                    </s:iterator>
                </select>
            </td>
            <td>职务：</td>
