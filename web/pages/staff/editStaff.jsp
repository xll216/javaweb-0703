...
<form action="${pageContext.request.contextPath}/hr/staffAction_edit.action" method="post">
    ...
    <table width="88%" border="0" class="emp_table" style="width:80%;">
        ...
        <tr>
            ...
            <td>性别：</td>
            <td>
                <input type="radio" name="gender" ${staff.gender=='男'?'checked':''} value="男"/>男
                <input type="radio" name="gender" ${staff.gender=='女'?'checked':''} value="女"/>女
            </td>
        </tr>
        <tr>
            <td width="10%">所属部门：</td>
            <td width="20%">
                <select id="post.department.depID"
                    ...
                    <s:iterator value="%{#departments}" var="list">
                        <option value="${list.depID}" ${list.depID==staff.post.department.depID?'selected':''}>${list.depName}</option>
                    </s:iterator>
                </select>
            ...
            <td width="62%">
                ...
                <select id="post.postId" name="post.postId">
                    ...
                    <s:iterator value="%{#allPost}" var="list">
                        <option value="${list.postId}" ${list.postId==staff.post.postId?'selected':''}>${list.postName}</option>
                    </s:iterator>
                </select>
            </td>
        </tr>
